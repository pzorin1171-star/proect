    ...
    screen.innerHTML = `
        <div style="text-align: center; padding: 50px;">
            ...
            <div style="margin: 40px 0;">
                <button onclick="showTeacherAuth()" style="font-size: 24px; padding: 20px 40px; margin: 20px; background: #4CAF50; color: white; border: none; border-radius: 10px; cursor: pointer;">
                    👨‍🏫 Режим учителя
                </button>
                <br>
                <button onclick="showStudentAuth()" style="font-size: 24px; padding: 20px 40px; margin: 20px; background: #2196F3; color: white; border: none; border-radius: 10px; cursor: pointer;">
                    👨‍🎓 Режим ученика
                </button>
            </div>
        </div>
    ...
    screen.innerHTML = `
        <div style="max-width: 400px; margin: 0 auto; padding: 20px;">
            ...
            <div style="border: 2px solid #4CAF50; padding: 20px; margin: 20px 0; border-radius: 10px;">
                ...
                <button onclick="teacherLogin()" style="width: 100%; padding: 12px; background: #4CAF50; color: white; border: none; border-radius: 5px; font-size: 16px; cursor: pointer;">Войти</button>
            </div>
            <div style="border: 2px solid #FF9800; padding: 20px; margin: 20px 0; border-radius: 10px;">
                ...
                <button onclick="teacherRegister()" style="width: 100%; padding: 12px; background: #FF9800; color: white; border: none; border-radius: 5px; font-size: 16px; cursor: pointer;">Зарегистрироваться</button>
            </div>
            <div id="teacher-message" style="color: red; margin: 10px 0; text-align: center;"></div>
            <button onclick="showModeSelection()" style="width: 100%; padding: 12px; background: #666; color: white; border: none; border-radius: 5px; font-size: 16px; cursor: pointer; margin-top: 20px;">← Назад</button>
        </div>
    `;
}
    ...
    screen.innerHTML = `
        <div style="max-width: 400px; margin: 0 auto; padding: 20px;">
            ...
            <div style="border: 2px solid #2196F3; padding: 20px; margin: 20px 0; border-radius: 10px;">
                ...
                <button onclick="studentLogin()" style="width: 100%; padding: 12px; background: #2196F3; color: white; border: none; border-radius: 5px; font-size: 16px; cursor: pointer;">Войти</button>
            </div>
            <div style="border: 2px solid #9C27B0; padding: 20px; margin: 20px 0; border-radius: 10px;">
                ...
                <button onclick="studentRegister()" style="width: 100%; padding: 12px; background: #9C27B0; color: white; border: none; border-radius: 5px; font-size: 16px; cursor: pointer;">Зарегистрироваться</button>
            </div>
...
function showTeacherPanel() {
    ...
    const studentsList = users.students.map(student => `
        <div style="border: 1px solid #ccc; padding: 15px; margin: 10px 0; border-radius: 8px;">
            ...
            <button onclick="viewStudentProgress('${student.username}')" style="padding: 8px 15px; background: #2196F3; color: white; border: none; border-radius: 4px; cursor: pointer;">Посмотреть прогресс</button>
        </div>
    `).join('');
    ...
    screen.innerHTML = `
        <div style="max-width: 800px; margin: 0 auto; padding: 20px;">
            ...
            <div style="border: 2px solid #FF9800; padding: 20px; margin: 20px 0; border-radius: 10px;">
                <h2>⚙️ Управление</h2>
                <button onclick="addTestAssignment()" style="padding: 10px 20px; margin: 5px; background: #FF9800; color: white; border: none; border-radius: 5px; cursor: pointer;">Добавить задание</button>
                <button onclick="viewClassStatistics()" style="padding: 10px 20px; margin: 5px; background: #9C27B0; color: white; border: none; border-radius: 5px; cursor: pointer;">Статистика класса</button>
            </div>
            <button onclick="logout()" style="padding: 12px 25px; background: #ff4444; color: white; border: none; border-radius: 5px; font-size: 16px; cursor: pointer; margin-top: 20px;">🚪 Выйти</button>
        </div>
    `;
}
    ...
    screen.innerHTML = `
        <div style="max-width: 600px; margin: 0 auto; padding: 20px; text-align: center;">
            ...
            <div style="margin: 30px 0;">
                <button onclick="startGame()" style="display: block; width: 100%; padding: 15px; margin: 10px 0; font-size: 18px; background: #4CAF50; color: white; border: none; border-radius: 8px; cursor: pointer;">🎯 Начать игру</button>
                <button onclick="showLevelSelect()" style="display: block; width: 100%; padding: 15px; margin: 10px 0; font-size: 18px; background: #2196F3; color: white; border: none; border-radius: 8px; cursor: pointer;">📋 Выбор уровня</button>
                <button onclick="showShop()" style="display: block; width: 100%; padding: 15px; margin: 10px 0; font-size: 18px; background: #FF9800; color: white; border: none; border-radius: 8px; cursor: pointer;">🏪 Магазин</button>
                <button onclick="showWorkshop()" style="display: block; width: 100%; padding: 15px; margin: 10px 0; font-size: 18px; background: #9C27B0; color: white; border: none; border-radius: 8px; cursor: pointer;">🔧 Мастерская</button>
                <button onclick="showScoreShop()" style="display: block; width: 100%; padding: 15px; margin: 10px 0; font-size: 18px; background: #E91E63; color: white; border: none; border-radius: 8px; cursor: pointer;">📊 Магазин оценок</button>
                <button onclick="showSettings()" style="display: block; width: 100%; padding: 15px; margin: 10px 0; font-size: 18px; background: #607D8B; color: white; border: none; border-radius: 8px; cursor: pointer;">⚙️ Настройки</button>
            </div>
            <button onclick="logout()" style="padding: 12px 25px; background: #ff4444; color: white; border: none; border-radius: 5px; font-size: 16px; cursor: pointer;">🚪 Выйти</button>
        </div>
    `;
}
...
function showWorkshop() {
    ...
    screen.innerHTML = `
        <div style="max-width: 600px; margin: 0 auto; padding: 20px;">
            <h2>🔧 Мастерская</h2>
            <p>Очки улучшения: <strong>${upgradePoints}</strong></p>
            <div style="border: 1px solid #ccc; padding: 15px; margin: 15px 0; border-radius: 8px;">
                <h3>Скорость</h3>
                <p>Уровень: <strong>${playerStats.speed}</strong></p>
                <button onclick="upgradeStat('speed')">Улучшить (10 очков)</button>
            </div>
            <div style="border: 1px solid #ccc; padding: 15px; margin: 15px 0; border-radius: 8px;">
                <h3>Прыжок</h3>
                <p>Уровень: <strong>${playerStats.jump}</strong></p>
                <button onclick="upgradeStat('jump')">Улучшить (10 очков)</button>
            </div>
            <div style="border: 1px solid #ccc; padding: 15px; margin: 15px 0; border-radius: 8px;">
                <h3>Прочность</h3>
                <p>Уровень: <strong>${playerStats.health}</strong></p>
                <button onclick="upgradeStat('health')">Улучшить (15 очков)</button>
            </div>
            <button onclick="showMainMenu()" style="padding: 12px 25px; background: #ff4444; color: white; border: none; border-radius: 5px; font-size: 16px; cursor: pointer; margin-top: 20px;">← Назад</button>
        </div>
    `;
}
...
function showScoreShop() {
    ...
    screen.innerHTML = `
        <div style="max-width: 600px; margin: 0 auto; padding: 20px;">
            ...
            <p>Ваши очки: <strong>${score}</strong></p>
            <div style="border: 1px solid #ccc; padding: 15px; margin: 15px 0; border-radius: 8px;">
                <h3>Оценка 5</h3>
...
function upgradeStat(stat) {
    ...
    if (upgradePoints >= cost) {
        ...
        playerStats[stat]++;
        saveGame();
        alert(`Улучшено до уровня ${playerStats[stat]}!`);
        showWorkshop();
    } else {
    ...
    levels.forEach((level, index) => {
        levelsHTML += `
            <div style="border: 1px solid #ccc; padding: 15px; margin: 10px 0; border-radius: 8px;">
                ...
                <button onclick="loadLevel(${index})" style="padding: 10px 20px; background: #4CAF50; color: white; border: none; border-radius: 5px; cursor: pointer;">Играть</button>
            </div>
        `;
    });
...
function loadLevel(levelIndex) {
    ...
    screen.innerHTML = `
        <div style="max-width: 600px; margin: 0 auto; padding: 20px; text-align: center;">
            <h2>🎮 Уровень ${levelIndex + 1}</h2>
            <p>Игровой процесс будет здесь...</p>
            <button onclick="completeLevel(${levelIndex})" style="padding: 12px 25px; background: #4CAF50; color: white; border: none; border-radius: 5px; font-size: 16px; cursor: pointer;">✅ Завершить уровень (тест)</button>
            <br>
            <button onclick="showLevelSelect()" style="padding: 12px 25px; background: #ff4444; color: white; border: none; border-radius: 5px; font-size: 16px; cursor: pointer; margin-top: 10px;">🚪 Выйти в меню</button>
        </div>
    `;
}
...
function viewStudentProgress(studentUsername) {
    ...
    screen.innerHTML = `
        <div style="max-width: 600px; margin: 0 auto; padding: 20px;">
            ...
            <button onclick="showTeacherPanel()" style="padding: 12px 25px; background: #ff4444; color: white; border: none; border-radius: 5px; font-size: 16px; cursor: pointer;">← Назад к панели</button>
        </div>
    `;
}
